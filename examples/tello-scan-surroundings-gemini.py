...
def process_voice_command(audio_text: str) -> dict:
    ...
    try:
        ...
        입력된 텍스트가 scan 명령에 해당하면 {"command": "scan"}을 반환하세요.
        ...
        입력: {text}
        """.format(text=audio_text)

        response = chat_model.generate_content(prompt)
        
        # 간단한 키워드 매칭으로 scan 명령 확인
        if any(keyword in audio_text for keyword in ["분석", "설명", "사진"]):
            return {"command": "scan"}
